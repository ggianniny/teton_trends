...
Checking for errors/exposure in 2019 and 2020:

```{r}
ggplot(filter(fully_cleaned6, site == "s_ak_basin", year ==2019|year==2020e), aes(x = date1, y = temp_c))+
  geom_point()+
  theme_classic()
```

Appears to be some very high/variable readings in both years. Isolating error periods: 

2019:

```{r}
ggplot(filter(fully_cleaned6, site == "s_ak_basin", date1 >= "2019-08-15" & date1 <= "2019-10-20"), aes(x = date_tm, y = temp_c))+
  geom_point()+
  theme_classic()
```
...
```{r}
ggplot(filter(fully_cleaned6, site == "s_ak_basin", date1 >= "2020-08-15" & date1 <= "2020-11-11"), aes(x = date_tm, y = temp_c))+
  geom_point()+
  theme_classic()
```
...
```{r}
full_clean_split <- split(fully_cleaned7, fully_cleaned7$site) # split into 17 df's, one for each site
```
...
tasr_summer_doy <- tasr_dat%>%
  ...
  mutate(source_print = case_when(source == "sub_ice" ~ "Icy Seep", source == "glacier" ~ "Glacier", source == "snowmelt" ~ "Snow Melt"))%>%
...
tasr_cumulative_hourly <- tasr_dat%>%
  mutate(source_print = case_when(source == "sub_ice" ~ "Icy Seep", source == "glacier" ~ "Glacier", source == "snowmelt" ~ "Snow Melt"),
      source_name = paste(source_print, full_name, sep = " - "))%>%
  group_by(year, site, source_name, source_print)%>%
  summarise(temp_cum = sum((na.omit(temp_c))))
```

Plot:

```{r}
temps.cumulative <- ggplot(tasr_cumulative_hourly, aes(x = year, y = temp_cum, color = source_name))+
  geom_point()+
  geom_line()+
  geom_smooth(method = "lm", linetype = "dashed", size = 0.4, se = F)+
  facet_wrap(~source_print, nrow = 3, ncol = 1, scales = "free")+
  scale_color_manual(values = boxplot.pal)+
  theme_classic()
temps.cumulative
...
temp.profiles <- tasr_daily%>%
  ...
  ggplot(aes(x = doy_dt, y = t_xbar, color = year_src))+
  geom_line()+
