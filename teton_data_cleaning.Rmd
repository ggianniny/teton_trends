...
```{r}
pb <- filter(fully_cleaned4, site == "paintbrush")%>% #select data for paintbrush
...
```{r}
ggplot(filter(fully_cleaned5, site == "paintbrush"), aes(x = date1, y = temp_c))+
  geom_point()+
  theme_classic()
...
```{r}
full_clean_split <- split(fully_cleaned5, fully_cleaned5$site) # split into 17 df's, one for each site
```
...
Add day of year and month column; extract data for July and August:

```{r}
tasr_summer_doy <- tasr_daily%>%
...
Draw plot of July-Aug data - line graph faceted by site x = day of year, y = temp, color = year

```{r}
tasr.doy <- ggplot(tasr_summer_doy, aes(x = doy, y = temp_xbar, color = factor(year)))+
  geom_line()+
  facet_wrap(~full_name, scales = "free", nrow = 2, ncol = 6)+
  theme_classic()+
  scale_color_manual(values = rev(brewer.pal(10, "RdYlGn")))+
  labs(x = "Day of Year", y = "Stream Temperature, C", color = "Year")+
...
```{r}
ggsave("pres_figures/tasr_stacked_years.pdf", tasr.lines, width = 12, height = 5, units = "in", device = "pdf", dpi = "retina")
```
