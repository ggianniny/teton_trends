...
snotel <- read.csv("teton_snotel_23.csv") #snotel data
...
stream_source <- merge(stream_temp, sources, all = T)%>%
  mutate(date1 = ymd(date1), 
         year = year(date1))%>% #Add source info to hourly temperature data; re-code date as r date format
  filter(!is.na(temp_c), #drop missing temp values
         !year == 2023) #DROP 2023 DATA - only had 2023 data for first half of August, so not relevant for analysis yet. 
...
Aside: Data export for Taylor: 

```{r}
julyTemps <- stream_source_daily %>%
  mutate(month = month(date1), 
         year = year(date1))%>%
  filter(month == 7)%>%
  group_by(site, full_name, source, year)%>%
  summarise(mean_jul_temp = mean(t_xbar), 
            mean_max_jul_temp = mean(t_max), 
            mean_min_jul_temp = mean(t_min), 
            mean_jul_temp_range = mean(t_range))

write.csv(julyTemps, "Temperature/cleaned_full_datasets/julyTemps.csv")
```
...
stream_aug <- stream_source_daily %>%
  mutate(mo = month(date1))%>% #add month column
  filter(mo == 8)%>% #extract July and August
  group_by(site, full_name, stream_code, year, lat, long)%>% #group by site and year
  summarise(summer_xbar = mean(t_xbar, na.rm = T), 
            max_xbar = mean(t_max, na.rm = T), 
            min_xbar = mean(t_min, na.rm= T), #calculate mean temperature, mean daily max and min
...
###### ii'. LMs of mean/min/max for August Only:

Nest plot data by source and measurement type:

```{r}
aug_plot <- as.data.frame(aug_source) %>%
  dplyr::select(source, year, t_xbar, t_max, t_min, t_range)%>%
  pivot_longer(-c(source, year), names_to = "measure", values_to = "temp")
...
```{r}
aug_source_pvals <- merge(aug.source.lms, aug_plot)%>% #add model info to source dataframe
  mutate(ypos = case_when(measure == "t_max" ~ 0.95, measure == "t_xbar" ~ 0.9, measure == "t_min" ~ 0.85, measure == "t_range"~0.8)) #create column for adjusting P-value label Y position
```
...
source.aug <- ggplot(aug_source_pvals, aes(x = year, y = temp, color = measure))+
  geom_point()+
  geom_line()+
  geom_smooth(se = F, method = "lm", linetype = "dashed", size = 0.5)+
  theme_classic()+
...
```{r}
ggsave("Temperature/plots/source_august.pdf", source.aug, device = "pdf", units = "in", height = 5, width = 6.5, dpi = "retina")
```
...
ggplot(lmm_pivot, aes(x = temp_c, fill = source))+
  geom_histogram(color = "black")+
  facet_wrap(~measure, scales = "free_x")+
  theme_classic()

#with log10 transform:
ggplot(lmm_pivot, aes(x = transform, fill = source))+
  geom_histogram(color = "black")+
  facet_wrap(~measure, scales = "free_x")+
  theme_classic()
...
All are left-skewed, but a log transform just makes them right-skewed. Skipping log transform for now. 
...
Significant negative correlation between melt length and max temp; same with max swe and max temp (??)
...
```{r}
max.lmm.viz<-ggplot(max_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  ...
  geom_text_npc(aes(npcx = 0.9, npcy = 0.1, label = paste( "P = ",round(p.value, 2), sep = "")), stat = "unique")+
  theme(legend.position = "none", 
        axis.title.x = element_blank())+
  labs(x = "", y = "Mean maximum July-Aug stream temp, C")
max.lmm.viz
```
...
Significant negative correlation between maximum SWE and min summer stream temp (higher SWE = lower minimum temps)

*Snowmelt:* 

Significant negative correlation between melt length, max SWE, and summer stream temp. (longer melt and higher max SWE = lower tream temps)

*Glacier:*

Significant negative correlation between max SWE and minimum temps
...
```{r}
min.lmm.viz<-ggplot(min_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
  ...
  theme(legend.position = "none", 
        axis.title.x = element_blank())+
  labs(x = "", y = "Mean minimum July-Aug stream temp, C")
min.lmm.viz
```
...
No significant correlations - closest is negative correlation between max SWE and avg stream temp. 

*Snowmelt:* 

Significant negative correlation Melt length, max SWE, and avg stream temp. 
...
```{r}
mean_pv <- lmm_plot %>%
  filter(measure == "summer_xbar")%>%
  merge(mean.lmm)
```

```{r}
mean.lmm.viz<-ggplot(mean_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
  ...
  theme(legend.position = "none", 
        axis.title.x = element_blank())+
  labs(x = "", y = "Mean daily July-Aug stream temp, C")
mean.lmm.viz
```
...
```{r}
range_pv <- lmm_plot %>%
  filter(measure == "range_xbar")%>%
  merge(range.lmm)
```


```{r}
tr.lmm<- ggplot(range_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
...
```{r}
lmm.viz.stack <- min.lmm.viz/mean.lmm.viz/max.lmm.viz/tr.lmm
lmm.viz.stack
```
...
```{r}
ggsave("Temperature/plots/lmm_visualizations.pdf", lmm.viz.stack, device = "pdf", units = "in", dpi = "retina", height = 11, width = 8.5)
```

### 4. August-only analysis (per Stefano's Feedback):

General goal: repeat LMM analyses using data from August only.

Calculate august stream temp metrics; drop sites with <3 years data:

```{r}
aug_temps <- stream_source_daily %>%
  mutate(mo = month(date1))%>% #add month column
  filter(mo == 8)%>% #extract August
  group_by(site, full_name, stream_code, year, lat, long)%>% #group by site and year
  summarise(summer_xbar = mean(t_xbar, na.rm = T), 
            max_xbar = mean(t_max, na.rm = T), 
            min_xbar = mean(t_min, na.rm= T), #calculate mean temperature, mean daily max and min
...
Add SNOTEL data; add columns with day of year for swe max and swe zero; calculate days since exposure: 

```{r}
aug_snotel <- merge(aug_temps, snotel_summary)%>% # merge
  mutate(swe_max_doy =yday(swe_max_date), 
         swe_zero_doy=yday(swe_zero_date), #add columns with day of year for swe max and zero
         days_exposed=yday(mdy("08-01-2024"))-swe_zero_doy) #Calculate days since snow cover
```

Test correlation between melt duration and days since snow cover: 

```{r}
summary(lm(days_exposed~melt_days, data = aug_snotel))
```

R2 = 0.5, so strong negative correlation between the two (longer melt duration = shorter time since exposure - not surprising). Test models with both, but probably can't include in same model: 

Data prep for plotting & modeling:

```{r}
aug_pivot <- aug_snotel %>%
  dplyr::select(max_xbar, min_xbar, range_xbar, summer_xbar, source, airtemp_summer, swe_max, melt_days, site, year, days_exposed)%>%
  pivot_longer(!c(source, airtemp_summer, swe_max, melt_days, site, year, days_exposed), names_to = "measure", values_to = "temp_c")%>%
  mutate(transform = log10(temp_c))
...
```{r}
aug_nested <- aug_pivot%>%
  nest(data = -c(source, measure))
```

#### 4A. Aug only, melt duration
...
```{r}
max_aug <- filter(aug_nested, measure == "max_xbar")
...
Significant negative correlation between max temp and max swe
...
```{r}
aug.max.lmm.viz<-ggplot(augmax_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
  ...
  theme(legend.position = "none", 
        axis.title.x = element_blank())+
  labs(x = "Predictor Value", y = "Mean maximum Aug stream temp, C")
aug.max.lmm.viz
```
...
```{r}
min_aug <- filter(aug_nested, measure == "min_xbar")
...
Significant negative correlation with max SWE

*Snowmelt:* 

Significant negative correlation between max SWE and stream temp

*Glacier:*

Significant positive correlation between summer air temp and min aug stream temp; negative with swe max
...
```{r}
aug.min.lmm.viz<-ggplot(augmin_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
  ...
  theme(legend.position = "none", 
        axis.title.x = element_blank())+
  labs(x = "Predictor Value", y = "Mean minimum Aug stream temp, C")
aug.min.lmm.viz
```
...
```{r}
mean_aug <- filter(aug_nested, measure == "summer_xbar")

aug.mean.lmm <- mean_aug %>%
  mutate(model = purrr::map(data, ~lmerTest::lmer(temp_c~melt_days+airtemp_summer+swe_max+(1|site), data = .)%>% #run a model of temp~year for each site (referenced by "."); store results in new column called "model"
                     broom.mixed::tidy(effects="fixed", conf.int = TRUE)))%>% #tidy function to construct a summary table with model results (seperate tables for each site)
  unnest(model)%>% #unnest model column to get rows for each site, columns for model metrics
  mutate(p.value = round(p.value, 4))%>%
...
Significant positive correlation with summer airtemp. 
marginally significant negative correlation with mean swe. 
...
```{r}
aug.mean.lmm.viz<-ggplot(augmean_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
  ...
  theme(legend.position = "none", 
        axis.title.x = element_blank())+
  labs(x = "Predictor Value", y = "Mean daily Aug stream temp, C")
aug.mean.lmm.viz
```
...
```{r}
aug_ranges <- filter(aug_nested, measure == "range_xbar")

aug.range.lmm <- aug_ranges %>%
  mutate(model = purrr::map(data, ~lmerTest::lmer(temp_c~melt_days+airtemp_summer+swe_max+(1|site), data = .)%>% #run a model of temp~year for each site (referenced by "."); store results in new column called "model"
                     broom.mixed::tidy(effects="fixed", conf.int = TRUE)))%>% #tidy function to construct a summary table with model results (seperate tables for each site)
  unnest(model)%>% #unnest model column to get rows for each site, columns for model metrics
  mutate(p.value = round(p.value, 4))%>%
...
```{r}
aug.range.lmm.plot <- ggplot(augrange_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
  ...
  theme(legend.position = "none")+
  geom_text_npc(aes(npcx = 0.9, npcy = 0.1, label = paste( "P = ",round(p.value, 2), sep = "")), stat = "unique")+
  labs(x = "Predictor Value", y = "Mean Aug daily stream temp. change, C")
aug.range.lmm.plot
```
...
```{r}
aug.lmm.viz.stack <- aug.min.lmm.viz/aug.mean.lmm.viz/aug.max.lmm.viz/aug.range.lmm.plot
aug.lmm.viz.stack
```
...
```{r}
ggsave("Temperature/plots/aug_lmm_visualizations.pdf", aug.lmm.viz.stack, device = "pdf", units = "in", dpi = "retina", height = 11, width = 8.5)
```

#### 4B. Aug only, days since snow cover

##### i. Maximum temp:

Run LMM for each source and measure - form: 

(t_max, t_min, t_range, t_ave) ~ airtemp_summer + swe_max + days_exposed + random(site)

```{r}
max_aug <- filter(aug_nested, measure == "max_xbar")

aug.max.exp <- max_aug %>%
  mutate(model = purrr::map(data, ~lmerTest::lmer(temp_c~days_exposed+airtemp_summer+swe_max+(1|site), data = .)%>% #run a model of temp~year for each site (referenced by "."); store results in new column called "model"
                     broom.mixed::tidy(effects="fixed", conf.int = TRUE)))%>% #tidy function to construct a summary table with model results (seperate tables for each site)
  unnest(model)%>% #unnest model column to get rows for each site, columns for model metrics
  mutate(p.value = round(p.value, 4))%>%
  filter(!term == "(Intercept)")%>%
...
```{r}
aug.max.exp.viz<-ggplot(augmax_exp_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
...
Significant negative correlation between max SWE and stream temp; positive between days exposed and stream temp. 
...
```{r}
aug.min.exp.viz<-ggplot(augmin_exp_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
...
Marginally significant positive correlation with summer airtemp. 

Visualization: 

```{r}
augmean_exp_pv <- aug_plot %>%
  filter(measure == "summer_xbar")%>%
  merge(aug.mean.exp)
```

```{r}
aug.mean.exp.viz<-ggplot(augmean_exp_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
...
```{r}
aug.range.exp.plot <- ggplot(augrange_exp_pv, aes(x = predictor_value, y = temp_c, color = source, group = site))+
  geom_point(size = 0.5)+
  geom_line(size = 0.5)+
  geom_smooth(aes(group = NULL), method = "lm", se = T, linetype = "dashed", color = "black", size = 0.5)+
  theme_classic()+
...
```{r}
aug.exp.viz.stack <- aug.min.exp.viz/aug.mean.exp.viz/aug.max.exp.viz/aug.range.exp.plot
aug.exp.viz.stack
```
...
```{r}
ggsave("Temperature/plots/aug_lmm_exposure.pdf", aug.exp.viz.stack, device = "pdf", units = "in", dpi = "retina", height = 11, width = 8.5)
```


#### 4C. Model Diagnostics

First, need to get lmer outputs for all of these models. Using a for loop: 

```{r}
aug_pivot<-mutate(aug_pivot, comb = paste(source, measure, sep = ", "))

mod.list <- list()
df.mod.list<-list()
diag.list<-list()
combs<-unique(aug_pivot$comb)
...
for(i in 1:length(combs)){
  temp<-filter(aug_pivot, comb == comb[i])
  mod<-lmerTest::lmer(temp_c~days_exposed+airtemp_summer+swe_max+(1|site), 
                                data = temp
                                  )
  df_mod <- broom.mixed::augment(mod)
  df_mod[".stdresid"] <- resid(mod, type = "pearson")
  
  df.mod.list[[i]]<-df_mod
  mod.list[[i]]<-mod
  
  resid<-ggplot(df.mod.list[[i]], aes(x = .fitted, y = .resid))+
    geom_point()+
    geom_hline(yintercept = 0)+
    labs(title = paste("Residuals", combs[i], sep = " - "))
  qq<- ggplot(df.mod.list[[i]], aes(sample = .stdresid)) +
  geom_qq() +
  geom_qq_line()+
    labs(title = paste("QQ Plot", combs[i], sep = " - "))
  diag.list[[i]]<-resid|qq
}
