...
Weak effect of source on area - which differences are significant? 

```{r}
TukeyHSD(source.aov)
```

Marginally significant difference between snowmelt and glacier (snowmelt = significantly larger area than glacier), no difference between subterranean ice and glacier or subterranean ice and snowmelt. 
...
richness <- invert_avgs %>% 
  ...
  group_by(Stream, Year)%>% #group by stream/year
...
Significant declines in richness at mid teton, s cascade, s teton. Could just be an artifact of the high richness values at all sites in 2016. 
...
No significant trends, which is unsurprising bc of the very limited data - trend may emerge with longer term dataset. 
...
yr_binary <- yr_taxa %>%
  mutate(across(.cols = c(2:8), to.binary)) #apply this function across all year columns in yr_taxa 
```
...
binary_lag <- binary_wide %>%
  mutate(across(c(2:94), apperance.calc))#apply to all taxa cols
...
```{r}
new_taxa <- rowSums(binary_lag[,2:94])
```

Add this to a new dataframe with year; calculate cumulative taxa observed each year: 

```{r}
cumulative_richness <- data.frame(c(2015:2021), new_taxa)%>% #bind new_taxa object with vector of years
...
Appears to be a steady increase in cumulative richness, but that could be a side effect of having data from more sites in later years. 
...
Significant (-) relationship between mean summer temp and richness at Gusher, not at any other sites. 
