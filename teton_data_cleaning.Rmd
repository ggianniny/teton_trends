...
temp_ranges_flagged <- full_temp_ranges%>%
  mutate(error = ifelse(t_range>(5*tr_sd), "Y", "N")) #Add column with temp range flag ("Y" if >5* SD)
...
ggplot(tr_updated_sd, aes(x = date1, y = t_range, color = site, fill = site))+
  ...
  facet_wrap(~site, scales = "free")+
  theme_classic()+
  geom_ribbon(aes(ymin = (tr_xbar_updated-(5*sd_updated)), 
                  ymax = (tr_xbar_updated+(5*sd_updated)),
                  alpha =0.5))+ #Add ribbons for 5* Temp Range SD
  geom_hline(aes(yintercept = tr_mean), linetype = "dashed")+ #add dashed line for mean temp range by site
  theme(legend.position = "none")+
  labs(x = "Date", y = "Daily temp. range, C")
...
#### iii. Extract date ranges with temp ranges >5x SD for each site

Flag temp values with SD > 5* mean, drop from dataset: 

```{r}
tr_flagged_final <- tr_updated_sd %>%
  mutate(error = ifelse(t_range>=(7*sd_updated), "Y", "N"), 
         month = month(date1))
...
tr.clean2 <- ggplot(
  ...
  theme_classic()+
  theme(legend.position = "none")+
  labs(x = "Date", y = "Daily temp. range, C", title = "Cleaned Data")+
  scale_y_continuous(limits=c(0,45))
...
```{r}
ggsave("Temperature/plots/sd_data_cleaning.pdf", tr.comparison, width = 6.5, height = 9, units = "in", device = "pdf", dpi = "retina")
```


**NOTES 5/12:** Tested a few different SD multiplier cutoffs: 

* 5*SD was cutting off too much + picking up too many sites/years. 
* 10* SD does a pretty good job, but maybe leaves some error measurements - but isolates 3 sites we had ID'd as having issues.
* 7* SD seems like a good compromise - still cuts a few measurements from Mid Teton, but only 24 obs from October 2023. Does a good job of removing problem areas from Cloudveil, Wind Cave, and S AK Basin. 
