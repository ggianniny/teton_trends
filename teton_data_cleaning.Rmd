...
Checking distribution of zeros across sites + years:

```{r}
neg.values <- clean_zero%>%
  mutate(date1=ymd(substr(date_tm, 1, 10)))%>% #add date column
  group_by(site, date1)%>%
  mutate(n_obs = length(temp_raw))%>%
  ungroup()%>%
  filter(temp_raw<0)%>% #extract negative readings
  group_by(site, date1)%>% #group by site and date
  summarise(n_neg = length(temp_raw), 
            n_obs = mean(n_obs))%>% #count number of negative temperature values per day for each site. 
  mutate(doy = yday(date1), 
         year=year(date1), 
         site_yr = paste(site, year, sep = " "), 
         doy_dt = as.Date(doy, origin = "2024-01-01"))

#Plotting:
neg.plot <- ggplot(neg.values, aes(x = doy_dt, y = n_neg, fill = factor(year)))+
  geom_bar(stat="identity", position = "dodge")+
  facet_wrap(~site)+
  theme_classic()+
  theme(legend.position = "right", 
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  labs(x = "Day of Year", y = "Number of hourly temp readings <0C", fill = "Year", title = paste("# Negative Readings per day by site ", "(", sum(neg.values$n_neg), "total neg. readings)"))+
  scale_x_date(date_breaks = "3 months", date_labels = "%b-%d")
neg.plot
```

Checking magnitude of <0 values (whole year and June-Sep):

```{r}
#Distribution of negative values (whole year): 
neg_data <- clean_zero %>%
  filter(temp_raw<0)%>%
  mutate(doy = yday(date_tm))%>%
  mutate(during_season = ifelse(doy>165&doy<259, "Y", "N"))

neg.cdf <- ecdf(neg_data$temp_raw)
plot(neg.cdf)

summary(neg_data$temp_raw)

#distribution of negative values (season only):



neg.season.cdf <- ecdf(
  (neg_data%>%
     filter(during_season == "Y"))$temp_raw
)
plot(neg.season.cdf)
...
Second plot: total # negative readings over time:

```{r}
neg_totals <- neg.values%>%
  group_by(site, doy_dt, doy)%>%
  summarise(neg_tot = sum(n_neg))%>%
  mutate(during_season = ifelse(doy>165&doy<259, "Y", "N"), 
         season_negs = ifelse(during_season == "Y", neg_tot, 0))

neg.duringseason <- sum(neg_totals$season_negs)
plot.title <- paste("# Negative Readings per day by site ", "(", sum(neg.values$n_neg), " total neg. readings; ", neg.duringseason," neg. readings June 15 - Sep. 15",")", sep ="")

neg.tot.plot <-ggplot(neg_totals, aes(x = doy_dt, y = neg_tot, fill = during_season))+
  geom_bar(stat = "identity", position = "dodge")+
  facet_wrap(~site)+
  theme_classic()+
  scale_x_date(date_breaks = "3 months", date_labels = "%b-%d")+
  scale_fill_manual(values = c("lightblue","maroon"), labels = c("No", "Yes"))+
  theme(legend.position = "right", 
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  labs(x = "Day of Year", y = "#hourly temp readings <0C", fill = "During Season (June 15 - Sep. 15)?", title = plot.title)
neg.tot.plot
...
```{r}
ggsave("Temperature/plots/neg_values_plot.jpg", plot = neg.tot.plot, device = "jpeg", width = 11, height = 8.5, dpi = "retina")
```
...
```{r}
ggsave("Temperature/plots/pb_dry_days.pdf", pb.drydays, width = 8.5, height = 6.5, units = "in", device = "pdf", dpi = "retina")
```
...
tr.xbar<- mean(s_ak$temp_range)
tr.sd<- sd(
  (filter(s_ak, !year==2019, !year==2020))$temp_range
  )

s_ak<-s_ak%>%
  mutate(tr_sd = ifelse(temp_range>(tr.xbar+(5*tr.sd)), ">5sd", "<5sd"))

s_ak.air <- merge(s_ak, airtemp, all = T)%>% #merge with air temps
  pivot_longer(!c(Date, year, temp_range, tr_sd), names_to = "measure", values_to = "temp_c")%>% #pivot longer for plotting
...
###### (Method used for final data cleaning)

First, check ranges & plot diel temp change over time:

```{r}
ggplot(s_ak, aes(x = Date, y =temp_range
                 #, color = tr_sd
                 ))+
  geom_line()+
  theme_classic()+
  geom_hline(aes(yintercept=mean(s_ak$temp_range)), linetype = "dashed")+
  geom_ribbon(aes(ymin = (mean(s_ak$temp_range))-(5*tr.sd), 
                  ymax = (mean(s_ak$temp_range))+(5*tr.sd)
                    ), alpha =0.5)+
...
```{r}
summary(s_ak$temp_range)
sd(s_ak$temp_range)

#Comparison with error and normal year:
sd(
  filter(s_ak, year == 2022)$temp_range
)

sd(
  filter(s_ak, year == 2019)$temp_range
)
```

Remove everything >5 SD:

```{r}
s_ak.1sd <- s_ak%>%
  filter(!tr_sd==">5sd")

ggplot(s_ak.1sd, aes(x = Date, y =temp_range
                 #, color = tr_sd
                 ))+
  geom_line()+
  theme_classic()+
  geom_hline(aes(yintercept=mean(s_ak.1sd$temp_range)), linetype = "dashed")+
  geom_ribbon(aes(ymin = (mean(s_ak.1sd$temp_range))-sd(s_ak.1sd$temp_range), 
                  ymax = (mean(s_ak.1sd$temp_range))+sd(s_ak.1sd$temp_range)
                    ), alpha =0.5)+
  labs(x = "Date", y = "Daily Temperature Range, C")

```

Extract date ranges with >5SD:
...
s.ak_toDrop <- s_ak %>%
  ...
  select(Date, year)%>%
  group_by(year)%>%
  mutate(id = seq(1:length(Date)))%>%
  ungroup()%>%
  pivot_wider(id_cols = id, names_from = year, values_from = Date)%>%
  rename("y2019"=2, "y2020"=3)

max(s.ak_toDrop$y2020, na.rm = T)
```

Drop from full data & check results: 

```{r}
fully_cleaned8 <- fully_cleaned7%>%
  filter(!(site=="s_ak_basin"&date1<max(s.ak_toDrop$y2020, na.rm = T)&date1>min(s.ak_toDrop$y2020, na.rm = T)),
         !(site=="s_ak_basin"&date1<max(s.ak_toDrop$y2019, na.rm = T)&date1>min(s.ak_toDrop$y2019, na.rm = T))
         )

s.ak.trClean<-ggplot(filter(fully_cleaned8, site == "s_ak_basin", year ==2019|year==2020), aes(x = date1, y = temp_c))+
  geom_point()+
  theme_classic()+
  labs(title = "Cleaned (removed obs with diel temp change >5SD from mean")+
  scale_y_continuous(limits = c(0, 15))
s.ak.trClean

s.ak.raw<-ggplot(filter(fully_cleaned7, site == "s_ak_basin", year ==2019|year==2020), aes(x = date1, y = temp_c))+
  geom_point()+
  theme_classic()+
  labs(title = "Raw")+
  scale_y_continuous(limits = c(0, 15))
...
NOT RUN: Remove problem periods from data:
...
### C'. Full dataset data cleaning: 

**Updated 5/12/2025:** Goal is to have the same data cleaning process for removing "problem" areas across all sites. Previous section used different methods for different sites (manual for Cloudveil and Wind Cave, comparison with air temp data for Paintbrush, Diel temp change for S AK Basin). Most likely candidate for applying across sites = remove observations with deil temp change >5xSD. 
...
#### i. Calculate daily temp ranges for each site & calcualte mean + SD for each site: 

```{r}
full_temp_ranges <- filled_clean %>%
...
Basic plot of temp ranges to identify suspected errors: 
...
Flag values/days with temp range >5SD: 
...
Plot of all data with SD bars, error points in red: 

```{r}
ggplot(temp_ranges_flagged, aes(x = date1, y = t_range, color = error))+
  geom_point()+
  facet_wrap(~site, scales = "free")+
  theme_classic()+
  geom_ribbon(aes(ymin = (tr_mean-(5*tr_sd)), 
...
Drop error points, re-plot data for error sites: 

```{r}
temp_ranges_cleaned <- temp_ranges_flagged %>%
  filter(!error == "Y") #drop points with temp range >5* SD for that site
```
...
#cleaned
tr.clean <- ggplot(
  filter(temp_ranges_cleaned, site%in%tr_errors$site), 
...
#raw
tr.raw <- ggplot(
  filter(full_temp_ranges, site%in%tr_errors$site), 
  aes(x = date1, y = temp_c, color = site)
)+
  geom_line()+
  facet_wrap(~site, scales = "free_x")+
...
Works for removing some of outliers, but not getting rid of all of them. Need to re-calculate SD without including error years. 

#### ii. Re-calculate SD excluding error years

**NOTE** - for now, treating any year that had measurements >5* original SD as an "error year" - this will ONLY work for sites with multiple years of data (i.e. not death canyon or schoolroom.)
...
#### iii. Drop data with >7 SD's over the mean
...
Might have made it too sensitive - picking up way more years from multiple sites. Test dropping error years + re-plotting: 

```{r}
tr_cleaned_final <- tr_flagged_final %>%
  filter(!error == "Y")

unique(tr_cleaned_final$site)
```
...
```{r}
ggsave("Temperature/plots/sd_data_cleaning.pdf", tr.comparison, width = 6.5, height = 6.5, units = "in", device = "pdf", dpi = "retina")
```
...
* 10 SD's probably leaves some error measurements in. 
...
* NOTE that this approach does NOT address issues with Paintbrush - will likely need to keep SNOTEL comparison cleaning for that site. 
...
##### a. Plotting for paper: 

Panel A: Temperature ranges with error values in red, band + dotted line for SD. 
...
Panel B & C: raw and cleaned data: 
...
ggsave("Temperature/plots/tr_cleaning_fig.jpg", tr.fig.stacked, device = "jpg", width = 6.5, height = 6.5, units = "in", dpi = "retina")
```


#### iv. Cleaning paintbrush using pre-established overlap periods: 

```{r}
fully_cleanedAlt <- tr_cleaned_final %>%
  filter(!(site == "paintbrush"&date1>=min(exposure.18)&date1<=max(exposure.18)), 
         !(site == "paintbrush"&date1>=min(exposure.19)&date1<=max(exposure.19)),
         !(site == "paintbrush"&date1>=min(exposure.20)&date1<=max(exposure.20)),
         !(site == "paintbrush"&date1>=min(exposure.21)&date1<=max(exposure.21)), 
         !(site == "paintbrush"&date1>=min(exposure.22)&date1<=max(exposure.22)), 
         !(site == "paintbrush"&date1>=min(exposure.23)&date1<=max(exposure.23))
         )%>% #dropping PB exposure dates
  select(date_tm, date1, site, year, temp_c, temp_raw) #dropping extra columns generated in data cleaning
```

Check with quick plot: 

```{r}
ggplot(fully_cleanedAlt, aes(x = date1, y = temp_c, color = site, fill = site))+
  geom_line()+
  facet_wrap(~site, scales = "free")+
  theme_classic()+
  theme(legend.position = "none")+
...
Fill in missing date values (grouped by site), check if successful:

```{r}
temps_final <- fully_cleanedAlt%>%
  group_by(site)%>% #group by site
...
sak.test<-filter(temps_final1, site == "s_ak_basin")
```
...
```{r}
boxplot.pal <- c(brewer.pal(3, "Purples"), brewer.pal(4, "Greens"), brewer.pal(4, "Oranges"))

summer.boxplot <- tasr_summer_doy%>%
  mutate(source_print = case_when(source == "sub_ice" ~ "Rock Glacier", source == "glacier" ~ "Glacier", source == "snowmelt" ~ "Snowfield"))%>%
  filter(!site == "windcave", !site == "paintbrush")%>%
...
```{r}
ggsave("Temperature/plots/warm_cold_lines.pdf", warmcold.lines, device = "pdf", units = "in", dpi = "retina", width = 8.5, height = 8.5)
```
...
tasr_doy <- tasr_dat %>%
  ...
  filter(doy>=122&doy<=245)%>% #extract DOY's 120-245 (May 1-Sep 1)
  ...
  arrange(source, site_yr, doy)


#Checking site/year combos:

tasr_doy%>%
  select(site_yr, source)%>%
  unique()%>%
  mutate(plot.no = c(1:92))%>%
  arrange(source, plot.no)
...
for(i in 1:length(site.yrs)){
  ...
  c<-cpop(t$t_xbar, t$doy, beta = 400) #feeding temperature and doy data into cpop, setting beta to 80 
...
snowmelt.cps<-(cp.plots[[26]]|cp.plots[[27]]|cp.plots[[28]]|cp.plots[[29]])/(cp.plots[[30]]|cp.plots[[31]]|cp.plots[[32]]|cp.plots[[33]])/(cp.plots[[34]]|cp.plots[[35]]|cp.plots[[36]]|cp.plots[[37]])/(cp.plots[[38]]|cp.plots[[39]]|cp.plots[[40]]|cp.plots[[41]])/(cp.plots[[42]]|cp.plots[[51]]|cp.plots[[52]]|cp.plots[[53]])/(cp.plots[[54]]|cp.plots[[55]]|cp.plots[[56]]|cp.plots[[57]])
...
```{r}
ggsave("Temperature/plots/snowmelt_changepoints.jpg", snowmelt.cps, device = "jpeg", units = "in", dpi = "retina", width = 11, height = 14)
```


Checking some glacier/sub_ice sites:

```{r}
other.cps<-(cp.plots[[9]]|cp.plots[[10]]|cp.plots[[11]]|cp.plots[[12]])/(cp.plots[[13]]|cp.plots[[14]]|cp.plots[[15]]|cp.plots[[16]])/(cp.plots[[17]]|cp.plots[[18]]|cp.plots[[19]]|cp.plots[[58]])/(cp.plots[[59]]|cp.plots[[60]]|cp.plots[[61]]|cp.plots[[62]])/(cp.plots[[63]]|cp.plots[[64]]|cp.plots[[65]]|cp.plots[[66]])/(cp.plots[[67]])
other.cps #check CPS for all one glacier/2 rock glacier sites
```

Save output plot:

```{r}
ggsave("Temperature/plots/other_changepoints.jpg", other.cps, device = "jpeg", units = "in", dpi = "retina", width = 11, height = 14)
```

Plot of change over time (Snowmelt): 

```{r}
snowmelt.cp.trends <- ggplot(filter(cps, source == "snowmelt", !site == "paintbrush", !is.na(cp_doy)), aes(x = year, y = cp_doy, color = site))+
  geom_point()+
  geom_line()+
...
Messy, but general downward trends in all three sites. 


## 8. Scatterplot: SNOTEL Air Temps (cumulative) vs. cumulative stream temps. 


General idea = cumulative degrees above zero (throughout year, just summer) for air and stream temps - sum up for each year, plot vs. eachother over time. 

Plot: x = stream temp, y = air temp, facet by site. 

First, calculate average air temp across both sites and then cumulative air temps by year, add to tasr_cumulative_hourly data file:  

```{r}
airtemp_cum <- airtemp%>%
  mutate(year = year(Date), #add date column
         temp_mean = ((targhee_c+phillips_c)/2))%>% #calculate mean daily air temp
  filter(temp_mean>=0)%>% #drop all temps <0
  group_by(year)%>% 
  summarise(airtemp_cum = sum(temp_mean))

head(airtemp_cum) #check results

air_stream_cum <- merge(tasr_cumulative_hourly, airtemp_cum)%>% #merge with cumulative temperature data
  mutate(airtemp_cum_hourly = airtemp_cum*24) #add column with cumulative air temp hours (multiply days *24)
head(air_stream_cum) #check results
```

Draw plot: x = stream temp, y = air temp, color = source, facet by site:

```{r}
stream.air.scatter <- ggplot(air_stream_cum, aes(x = temp_cum, y = airtemp_cum_hourly, color = source_name))+
  geom_point()+
  geom_line()+
  theme_classic()+
  labs(x = "Cumulative annual degree hours >0 C - Stream", y = "Cumulative annual degree hours >0 C - Air", color = "Source Type")+
  theme(
    axis.title = element_text(size = 11, face = "bold"),
    legend.title = element_text(size = 11, face = "bold"), 
    legend.position = "bottom", 
  ...
  scale_color_manual(values = boxplot.pal
                       #c("#756BB1", "#74C476", "#D94701")
                     )+
  facet_wrap(~source_print, scales = "fixed", nrow = 4, ncol =4)
stream.air.scatter
```
...
```{r}
ggsave("Temperature/plots/airtemp_streamtemp_scatter.jpg", stream.air.scatter, device = "jpeg", units = "in", dpi = "retina", width = 11, height = 8.5)
```


## 9. Plot showing snowmelt vs. glacier vs. rg summer temp regiems: 
 
Prep data and draw plot:

```{r}
summer.comparison <- tasr_dat%>%
  filter(!site == "paintbrush", !site=="windcave")%>%
  mutate(month = month(date1), year = year(date1))%>%
  filter(month == 6|month==7|month==8|month==9)%>%
  filter(year == 2023)%>%
  mutate(source_print = case_when(
    source=="sub_ice"~"Rock glacier", 
    source=="glacier"~"Glacier",
    source=="snowmelt"~"Snow field"
  ), 
  full_name = case_when(
    site=="cloudveil"~"Cloudveil", 
    site=="delta" ~ "Delta", 
    site == "grizzly"~"Grizzly", 
    site == "gusher"~"Gusher", 
    site == "mid_teton"~"Middle Teton", 
    site == "n_teton"~"North Teton", 
    site == "s_ak_basin"~"South AK Basin", 
    site == "s_cascade"~ "South Cascade", 
    site == "s_teton"~"South Teton", 
    site == "skillet"~"Skillet"
  ))%>%
  mutate(src_name = paste(source_print, full_name, sep = " - "))%>%
  ggplot(aes(x = date_tm, y = temp_raw, color = src_name))+
  geom_line()+
  theme_classic()+
  labs(x = "Date", y = "Stream temperature, C", color = "Stream name & source type")+
  theme(
    axis.title = element_text(size = 11, face = "bold"),
    legend.title = element_text(size = 11, face = "bold"), 
    legend.position = "right", 
...
```{r}
ggsave("Temperature/plots/summer_comparison.jpg", summer.comparison, device = "jpeg", units = "in", dpi = "retina", width = 6.5, height = 4.5)
```

Second plot - Boxplot by source type for all years of data: 

```{r}
summer.comparison.box <- tasr_dat%>%
  filter(!site == "paintbrush", !site=="windcave")%>%
  mutate(month = month(date1), year = year(date1))%>%
  filter(
    #month == 6|month==7|
  ...
  mutate(src_name = paste(source_print, full_name, sep = " - "))%>%
  ggplot(aes(x = source_print, y = temp_raw, fill = source_print))+
  geom_boxplot()+
  #geom_jitter()+
  theme_classic()+
  ...
  scale_fill_manual(values = c("purple", "lightgreen", "orange"))
...
```{r}
ggsave("Temperature/plots/summer_comparison_box.jpg", summer.comparison.box, device = "jpeg", units = "in", dpi = "retina", width = 6.5, height = 3.5)
```
