...
brm1 <- readRDS("Temperature/brms_models/all_sites/june/fit_rand_slopes_aug.rds")
# plot(brm1)
# pairs(brm1)
...
my_ggsave(plot = fit_spag,
       filename = paste(write_dir, "/fitted_spaghetti_june.png", 
                        sep = ""))
...
write_csv(prob_pos_df,
          file = paste(write_dir, "/prob_slope_positive_june.csv", 
                       sep = ""))
...
my_ggsave(plot = source_slope_halfeye,
       filename = paste(write_dir, "/source_slope_halfeye_june.png", 
                        sep = ""))
...
#           file = paste(write_dir, "/slope_magnitudes_june.csv", 
#                        sep = ""))
...
source_spag <- add_epred_draws(newdata = source_data,
  ...
  facet_wrap(~source) +
  theme_bw() +
  labs(title = "Fitted June Temperatures in GTNP",
...
my_ggsave(plot = source_spag,
       filename = paste(write_dir, "/source_slope_spag_june.png", 
                        sep = ""))
