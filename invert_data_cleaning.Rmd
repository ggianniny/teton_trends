...
nmds.site <- ggplot(mds_scores)+
  ...
  scale_shape_manual(values = c(15, 16, 17, 3, 4, 18, 6, 7, 5, 11))+ #setting shape palatte
...
yr_binary <- yr_taxa %>%
  mutate(across(.cols = c(2:ncol(yr_taxa)), to.binary)) #apply this function across all year columns in yr_taxa 
```
...
```{r}
cumulative_richness <- data.frame(c(2015:2024), new_taxa)%>% #bind new_taxa object with vector of years
