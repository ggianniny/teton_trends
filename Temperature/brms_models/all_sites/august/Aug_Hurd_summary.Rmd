...
temp_aug <- temp_clean %>% 
  ...
  filter(month = 8) |>
  mutate(temp_1 = temp_c / max(temp_c)) # scale temperature from 0-1 by dividing by max
```
...
$$logit(\theta) = \alpha_0 + \alpha_j + \alpha_k + \beta_{year}*year + \beta_{source}*source + \beta_{y*s} * year:source + \beta_{year_{[k]}} * year$$

$$\binom{\alpha_k}{\beta_{year_{[k]}}} \sim MVN(\binom{\alpha}{\beta_{year_{}}},  \Sigma)$$  


$$\mathbf \Sigma = \begin{bmatrix} \sigma_{\alpha} & 0 \\ 0 & \sigma_{\beta} \end{bmatrix} R \begin{bmatrix} 1 & \rho \\ \rho & 1 \end{bmatrix}$$
...
Global effect of source  
$\beta_{source} \sim N(0, 0.5)$

Correlation of year effect per site  
$\beta_{year_{[k]}} \sim N(0, 0.5)$

$\rho \sim LKJ(1)$

$shape \sim \Gamma(0.01, 0.01)$

Random intercept offset for year
$\alpha_{j} \sim N(0, \sigma_j)$

Random intercept offset for site
