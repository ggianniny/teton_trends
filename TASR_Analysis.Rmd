...
```{r}
stream_info <- summer_3plus %>%
  select(full_name, year, source)%>% #select full name, year, and source columns
...
map_dat <- as.data.frame(summer_3plus)%>%
  select(full_name, source, lat, long)%>%
  distinct()%>%
...
warming_amounts <- source.lms %>%
  ...
  select(source, measure, deg_decade, p.value)%>%
...
### 3. Overall trends in stream temperature over time - Time series analysis
